...
cursor.execute('''CREATE TABLE IF NOT EXISTS animes (anime_id INTEGER PRIMARY KEY, title TEXT, year TEXT, 
                                                     seasons INTEGER, genre_id INTEGER, rating FLOAT, 
                                                     rewatch_value INTEGER, owned BOOLEAN, manga_id INTEGER, 
                                                     notes TEXT)''')
cursor.execute('''CREATE TABLE IF NOT EXISTS mangas (manga_id INTEGER PRIMARY KEY, title TEXT, author TEXT, 
                                                     illustrator TEXT, year TEXT, chapters INTEGER, 
                                                     genre_id INTEGER, rating FLOAT, owned BOOLEAN, 
                                                     anime_id INTEGER, notes TEXT)''')
...
genres = ["mecha", "fantasy", "horror", "isekai", "slice of life", "shojo", "seinen", "josei",
          "shonen", "sports", "comedy", "romance", "high school", "Sci-Fi"]
...
cursor.execute('''INSERT INTO animes(title, year, seasons, genre_id, rating, rewatch_value, owned, manga_id, notes)
               VALUES ('Attack On Titan', '2013-2023', 4, 10, 9.9, 4, TRUE, 2, 'Attack on Titan has received universal 
...
cursor.execute('''INSERT INTO animes(title, year, seasons, genre_id, rating, rewatch_value, owned, manga_id, notes)
               VALUES ('Steins;Gate', '2011-2018', 2, 15, 9.7, 5, FALSE, null, 'Steins;Gate received critical acclaim, 
...
cursor.execute('''INSERT INTO animes(title, year, seasons, genre_id, rating, rewatch_value, owned, manga_id, notes)
               VALUES ('The Melancholy of Haruhui Suzumiya', '2009', 1, 2, 9.3, 5, TRUE, null, 'Kyon is a sardonic, 
...
cursor.execute('''INSERT INTO animes(title, year, seasons, genre_id, rating, rewatch_value, owned, manga_id, notes)
               VALUES ('Naruto', '2002-2007', 2, 9, 8.2, 1, FALSE, null, 'A powerful fox known as the Nine-Tails attacks 
...
cursor.execute('''INSERT INTO mangas(title, author, illustrator, year, chapters, genre_id, rating, owned, anime_id, notes)
               VALUES ('Death Note', 'Tsugumi Ōba', 'Takeshi Obata', '2003-2006', 108, 2, 9.5, FALSE, null, 'Death Note 
...
cursor.execute('''INSERT INTO mangas(title, author, illustrator, year, chapters, genre_id, rating, owned, anime_id, notes)
               VALUES ('Attack on Titan', 'Hajime Isamaya', 'Hajime Isayama', '2009-2021', 139, 9, 9.8, TRUE, 1, 'Attack 
               ...
               walls.')''')
cursor.execute('''INSERT INTO mangas(title, author, illustrator, year, chapters, genre_id, rating, owned, anime_id, notes)
               VALUES ('Jojo''s Bizarre Adventure', 'Hirohiko Araki', 'Hirohiko Araki', '1987-Present', 138, 9, 8.8, 
                       FALSE, null, 'JoJo''s Bizarre Adventure is well known for its art style and poses, frequent 
